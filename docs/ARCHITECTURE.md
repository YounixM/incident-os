...
`propose_rollback` is the write that opens the human approval dialog. For ChatGPT / site-tool calls it **waits until Approve or Cancel**, then returns `status: approved`. Read tools never open that dialog. If ChatGPT never calls `propose_rollback`, the page does not invent an approval — the human or agent must propose. Approving with no waiter executes `rollback_deployment`. In-app investigation still returns immediately and uses `waitForApproval`. `get_incident` includes `approval.approved` as a poll fallback if the wait is cut short, and reports the workspace incident status (investigating → resolved). After rollback, later tool calls stay visible; summaries label historical traces/logs as the incident window. `rollback_deployment` still requires a matching approved pending action. ChatGPT also runs its own safety review before each invocation.
